...
class OpenAIImageProvider(ImageProvider):
    ...
    def generate_image(
        ...
        try:
            ...
            # Try message.images field (OpenRouter returns images here)
            if hasattr(message, 'images') and message.images:
                for img_part in message.images:
                    if isinstance(img_part, dict):
                        ...
                        url = getattr(img_part.image_url, 'url', '') if hasattr(img_part.image_url, 'url') else img_part.image_url.get('url', '')
                        if url.startswith('data:image'):
                            base64_data = url.split(',', 1)[1]
                            image_data = base64.b64decode(base64_data)
                            image = Image.open(BytesIO(image_data))
                            logger.debug(f"Successfully extracted image from message.images (obj): {image.size}, {image.mode}")
                            return image
            ...
            logger.warning(f"Message has images attr: {hasattr(message, 'images')}")
