...
def update_page_outline(project_id, page_id):
    ...
    {
        "outline_content": {"title": "...", "points": [...]},
        "sync_description": true  // 可选，是否同步更新描述中的标题
    }
    """
    try:
        page = Page.query.get(page_id)
        ...
        old_outline = page.get_outline_content()
        new_outline = data['outline_content']
        
        page.set_outline_content(new_outline)
        page.updated_at = datetime.utcnow()
        
        # 如果启用了同步，且标题发生变化，更新描述中的标题
        sync_description = data.get('sync_description', True)
        if sync_description:
            old_title = old_outline.get('title', '') if old_outline else ''
                ...
                if desc_content:
                    ...
                    if desc_text:
                        ...
                        for pattern in title_patterns:
                            desc_text = re.sub(pattern, r'\1' + new_title, desc_text, count=1)
                        
                        desc_content['text'] = desc_text
...
def update_page_description(project_id, page_id):
    ...
    Request body:
    {
        ...
        "sync_outline": true  // 可选，是否从描述中提取标题同步到大纲
    }
    """
    try:
        page = Page.query.get(page_id)
        ...
        new_desc_content = data['description_content']
        page.set_description_content(new_desc_content)
        page.updated_at = datetime.utcnow()
        
        # 如果启用了同步，尝试从描述中提取标题并更新大纲
        sync_outline = data.get('sync_outline', True)
        if sync_outline:
            # 尝试从描述文本中提取标题
            desc_text = new_desc_content.get('text', '')
            if desc_text:
                import re
                # 尝试匹配"主标题："或"页面标题："后的内容
                title_match = re.search(r'(?:主标题|页面标题)[：:]\s*([^\n]+)', desc_text)
                if title_match:
                    new_title = title_match.group(1).strip()
                    outline_content = page.get_outline_content()
                    if outline_content:
                        outline_content['title'] = new_title
...
@page_bp.route('/<project_id>/pages/<page_id>/sync', methods=['PUT'])
def sync_page_content(project_id, page_id):
    """
    PUT /api/projects/{project_id}/pages/{page_id}/sync - 同时更新大纲和描述，并自动同步
    
    Request body:
    {
        "outline_content": {"title": "...", "points": [...]},  // 可选
        "description_content": {"text": "..."},  // 可选
        "sync_direction": "both"  // "outline_to_desc" | "desc_to_outline" | "both"
    }
    
    如果同时提供了outline_content和description_content，会根据sync_direction决定同步方向：
    - "outline_to_desc": 大纲标题同步到描述
    - "desc_to_outline": 描述标题同步到大纲
    - "both": 双向同步（默认）
    """
    try:
        page = Page.query.get(page_id)
        ...
        if 'outline_content' in data:
            ...
            # 如果启用了同步，且标题发生变化，更新描述中的标题
            if sync_direction in ('outline_to_desc', 'both'):
                old_title = old_outline.get('title', '') if old_outline else ''
                new_title = new_outline.get('title', '')
        ...
        if 'description_content' in data:
            ...
            # 如果启用了同步，从描述中提取标题并更新大纲
            if sync_direction in ('desc_to_outline', 'both'):
                desc_text = new_desc_content.get('text', '')
                if desc_text:
                    import re
                    # 尝试匹配"主标题："或"页面标题："后的内容
        ...
        db.session.rollback()
        logger.error(f"sync_page_content failed: {str(e)}", exc_info=True)
        return error_response('SERVER_ERROR', str(e), 500)
